...
function isValidVitals(vitals) {
    const { bodyTemp, pulseRate, respirationRate, bloodOxygen, glucoseLevel } = vitals;
    return bodyTemp > 0 && pulseRate > 0 && respirationRate > 0 && bloodOxygen > 0 && glucoseLevel > 0;
}
...
function isValidMeta(body) {
    return (isValidPatientId(body.patientId) && checkTimestampFormat(body.timestamp) && isValidVitals(body.block.vitals) && isValidSignature(body));
}
...
app.post('/createBlock', function (req, res) {
    isEndpointEnabled(req, res, () => {
        ...
        if (!isMasterNode) {
            ...
            log(`Creating block for patient ${req.body.patientId} and broadcasting to network`);
            log(JSON.stringify(req.body));
            ...
            log(`Timestamp used: ${timestamp}`);
            const createdBlock = blockchain.createBlock(blockchain.getLastBlock()['hash'], req.body.patientId, req.body.block, timestamp);

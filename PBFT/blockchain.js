...
function Blockchain() {
    ...
    // Updated genesis block for patient data
    const genesisBlock = this.createBlock("PatientChainGenesisBlock", "-", {
        data: "I am the genesis block of the Patient Chain!",
    });
...
Blockchain.prototype.getBlockHash = function(previousBlockHash, patientId, patientData) {
    const dataAsString = previousBlockHash + JSON.stringify(patientId) + JSON.stringify(patientData);
    return sha256(dataAsString);
}

Blockchain.prototype.createBlock = function(lastBlockHash, patientId, patientData, timestamp) {
    var index = 1;
    try { 
    ...
    // Updated block structure for patient data
    return {
        index: index,
        id: uuid(),
        timestamp: timestamp,
        ...
        hash: this.getBlockHash(lastBlockHash, patientId, patientData),
...
Blockchain.prototype.isValidNewBlock = function(newBlock) {
    ...
    const recalculatedNewHash = this.getBlockHash(newBlock['previousBlockHash'], newBlock['patientId'], newBlock['patientData']);
